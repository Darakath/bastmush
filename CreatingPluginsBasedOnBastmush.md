# Introduction #
Creating your own plugins based on Bastmush

# Process #
  * Create a directory under worlds/plugins for the new plugins (such as Test)
  * add this file into the directory as findbast.lua
```
-- finds Bast's plugins
...
nfile = scan_dir_for_file (GetInfo(60), "miniwin.lua", true)

if nfile then
  -- add the lua directory from Bast's plugins to the lua search path
  package.path = nfile .. "?;" .. nfile .. "?.lua;" .. package.path
else
  print("Could not find Bast's scripts, please install Bast's scripts into a directory under worlds/plugins.")
  print("the website is http://bastmush.googlecode.com")
end
```
  * Use this sample plugin and save it to the same directory and see [miniwin](miniwin.md) for help on styles
```
...
<muclient>
<plugin
   name="miniwin_ex_for_plugin_authors"
   author="Bast"
   id="eee8adec415c1bbb534ef093"
   language="Lua"
...
<script>
<![CDATA[
dofile (GetPluginInfo (GetPluginID (), 20) .. "findbast.lua")
...
testwin = Miniwin:new{name="New Example", resizable=true}
